			...
			<div class="card-body">
				<div class="table">
					<div id="dataTable_wrapper"
						...
						<div class="row">
							<div class="col-md-3">
								<form method="get" action="quan-ly-giang-vien"
							...
							<div class="col-md-9">
								<a
									href="/quanlysinhvien_hibernate/quan-ly-giang-vien?action=themmoigiangvien"
						...
						<div class="row">
							<div class="col-sm-12">
								...
								<table class="table table-bordered dataTable" id="dataTable"
									...
									<tbody>
										<c:forEach var="listGiangVien" items="${listGiangVien}">
											<tr role="row" class="odd">
												<td>${listGiangVien.magiangvien }</td>
												<td>${listGiangVien.tengiangvien }</td>
												<td><c:choose>
														<c:when test="${listGiangVien.gioitinh==1}">
															<span>Nam</span>
														</c:when>
														<c:otherwise>
															<span>Nữ</span>
													...
													<td>${listGiangVien.ngaysinh }</td>
													<td>${listGiangVien.diachi }</td>
													<td>${listGiangVien.email }</td>
													<td>${listGiangVien.sodienthoai }</td>
													<td>${listGiangVien.khoa.tenkhoa }</td>
												<td><a
													href="/quanlysinhvien_hibernate/quan-ly-giang-vien?action=suagiangvien&magiangvien=${listGiangVien.magiangvien }">Sửa</a>|<a
													href="#" data-id="${listGiangVien.magiangvien }" class="xoa">Xóa</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
											...
											<c:choose>
												<c:when test="${search!=null}">
													<li class="paginate_button page-item " id="${i}"><a
														href="/quanlysinhvien_hibernate/quan-ly-giang-vien?search=<c:out value="${search}" />&page=<c:out value="${i}" />"
														aria-controls="dataTable" data-dt-idx="${i }" tabindex="0"
														class="page-link"><c:out value="${i}" /></a></li>
												</c:when>
												<c:otherwise>
													<li class="paginate_button page-item " id="${i}"><a
														href="/quanlysinhvien_hibernate/quan-ly-giang-vien?page=<c:out value="${i}" />"
														aria-controls="dataTable" data-dt-idx="${i }" tabindex="0"
														class="page-link"><c:out value="${i}" /></a></li>
												</c:otherwise>
											</c:choose>
